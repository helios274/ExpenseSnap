...
var sources = document.querySelectorAll('input[type="checkbox"]');
var checkedSources = [];
...
filterBtn.addEventListener("click", (e) => {
  e.preventDefault();
  if (checkedSources.length !== 0) checkedSources = [];
  sources.forEach(function (source) {
    if (source.checked) {
      checkedSources.push(source.value);
    }
  });
  if (
    ...
    .then((data) => {
      ...
      if (data.length === 0) {
        ...
        data.forEach((item) => {
          ...
          <a
          href="update-income/${item.id}"
          class="btn btn-sm btn-dark me-2"
          ><i class="bi bi-pencil-square"></i></a>
          <a
            href="delete-income/${item.id}"
